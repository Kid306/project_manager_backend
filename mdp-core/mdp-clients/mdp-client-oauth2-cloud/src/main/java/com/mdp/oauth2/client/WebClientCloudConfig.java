...
public class WebClientCloudConfig {

	//@Value("${spring.application.name:}")
	String defaultClientRegistrationId="def-client";

	@Autowired
	List<LoadBalancedExchangeFilterFunction> loadBalancedExchangeFilterFunctionList;
	...
	WebClient webClient(OAuth2AuthorizedClientManager authorizedClientManager) {
		...
		oauth2.setDefaultClientRegistrationId(defaultClientRegistrationId);
			return WebClient.builder()
					...
					.filters(x->x.addAll(loadBalancedExchangeFilterFunctionList))
					.build();
	}
